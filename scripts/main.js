...
/* =========================================================
   ...
   Fixes:
   - globe click not opening
   - immediate close due to global document click handlers
   - pointer-events / z-index blocking
   ========================================================= */
(() => {
  "use strict";
  ...
  function bind() {
    ...
    // 1) Force above everything + clickable
    sw.style.zIndex = "2147483647";
    sw.style.pointerEvents = "auto";
    toggle.style.pointerEvents = "auto";
    rail.style.pointerEvents = "auto";

    // 2) Inject CSS override (beats demo-mode / global rules)
    ...
    // 3) Hard toggle on CAPTURE + stopImmediatePropagation
    // This beats any other onclick/document click close logic.
    toggle.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    // 4) Clicking inside the rail must not close it
    rail.addEventListener("click", (e) => {
      e.stopPropagation();
      e.stopImmediatePropagation();
    }, true);

    // 5) Clicking outside closes (capture)
  ...
  // Bind now, or retry briefly if DOM isn't ready yet
  if (bind()) return;
  let tries = 0;
  const t = setInterval(() => {
    tries++;
...
(() => {
  ...
  const t = setInterval(() => {
/* =========================================================
   EPTEC OVERLAY SANITIZER + LANG UNBLOCK (append-only)
   Goal:
   - If any overlay is accidentally covering the UI, make it non-interactive
   - Ensure language switcher is always above and clickable
   - No replacement, no refactor, only append
   ========================================================= */
(() => {
  "use strict";
  ...
  function sanitizeOnce() {
    ...
    // 1) Hard-force language switcher above everything
    if (sw) {
    ...
    // 2) If overlays exist, make sure "hidden" truly means non-interactive
    const tunnel = $("eptec-tunnel");
    const flash = $("eptec-white-flash");
    const paywall = $("paywall-screen");

    // Tunnel: if hidden -> disable interaction completely
    if (tunnel) {
      const hidden = tunnel.classList.contains("tunnel-hidden");
      if (hidden) {
        tunnel.style.pointerEvents = "none";
        tunnel.style.display = "none";
      } else {
        tunnel.style.pointerEvents = "auto";
        tunnel.style.display = "block";
      }
    }

    // White flash: if hidden -> disable interaction completely
    if (flash) {
      const hidden = flash.classList.contains("white-flash-hidden");
      if (hidden) {
    ...
    // Paywall: if hidden -> disable interaction completely
    if (paywall) {
      const hidden = paywall.classList.contains("paywall-hidden");
      if (hidden) {
        paywall.style.pointerEvents = "none";
        paywall.style.display = "none";
      } else {
        paywall.style.pointerEvents = "auto";
        paywall.style.display = "flex";
      }
    }

    // 3) As a last safety: inject CSS override (beats any accidental global CSS)
    if (!document.getElementById("eptec-overlay-sanitize-style")) {
      const style = document.createElement("style");
      style.id = "eptec-overlay-sanitize-style";
      style.textContent = `
        #language-switcher, #language-switcher * { pointer-events: auto !important; }
        #language-switcher { z-index: 2147483647 !important; }
        #eptec-tunnel.tunnel-hidden,
        #eptec-white-flash.white-flash-hidden,
        #paywall-screen.paywall-hidden {
  ...
  // run multiple times (because overlays can appear after boot)
  function run() {
    sanitizeOnce();
    setTimeout(sanitizeOnce, 100);
    setTimeout(sanitizeOnce, 400);
    setTimeout(sanitizeOnce, 1000);
  }
